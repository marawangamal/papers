...
def _clean_text(x):
    if x is None:
        return None
    # force string, strip nulls and normalize whitespace
    s = str(x).replace("\x00", "")
...
def _clean_list(xs):
    return [ _clean_text(x) for x in xs if x is not None ]
...
class ICLRScraper(ConferenceScraper):
    ...
    def base_url(self, year: int = None) -> str:
        api_prefix = (
            "api" if year is not None and year <= self.API_V1_THRESHOLD else "api2"
        )
    ...
    def get_papers(self, year: int) -> List[Dict]:
        ...
        try:
            if (
                year <= self.API_V1_THRESHOLD
                and year != self.API_V1_GET_PARAMS_EXCEPTION
            ):
            ...
            while True:
                ...
                if total_count < count:
                    url = (
                        f"{self.base_url(year)}/notes?{get_params}&offset={total_count}"
                    )
            ...
            if year <= self.API_V1_THRESHOLD:
                ...
                paper_data = [
                    ...
                    if "submitted"
                    not in paper.get("content", {}).get("venue", "").lower()
                ]
                ...
                if year != self.API_V1_GET_PARAMS_EXCEPTION:
                    paper_data = [
                        ...
                        if any(
                            [
                                "accept"
                                in reply.get("content", {}).get("decision", "").lower()
                                or "accept"
                                in reply.get("content", {})
                                .get("recommendation", "")
                                .lower()
                                for reply in paper.get("details", {}).get(
                                    "directReplies", []
                                )
                            ]
                        )
                ...
                tqdm.write(
                    f"Retained only {len(paper_data)} accepted papers for {year}."
                )
            return paper_data
